...
文章说的是前端有多难，很多人都觉得这些标签简单。然而想象一下，要写多少的标签才能理解**语义化**的意义？要写多少页面才能真正的明白这个节点应该写什么标签？

## CSS

然后是关于 CSS，我觉得这方面是很复杂的。
...
都说二流程序员爱玩工具，那我应该算是三流程序员了。对于`vim`如果我不算狂热，那可能没有几个人算真诚了。
...
到现在，HTTP2大行其道，可是前端却没有一款适合HTTP2的打包工具。所以我觉得半年到一年左右还会有新的工具出现，大家准备好重新学习吧 :)
...
现在不写个 react 好意思跟人说自己是写前端么！写了react怎么说也得接触一下redux吧。然后就完了。。。和之前完全不同的新思维就来了。

之前jQuery绑定一个数据直接改就好了。现在出个单项数据流，什么操作都得发个action，视图得订阅store。好吧，这还不算FP的范畴。

高阶函数总是了吧。我之前从来没想过高阶函数怎么在react里面用，直到看了 connect 源码吃了一惊，卧槽，还能这么写！

还有说烂了的柯里化。

现在还出现了很多js的函数式语言变种，就像Elm, cj什么的。
...
原来前端不用关心的东西现在都得考虑。比较Node和前端靠的近，人家问起来自己不会多尴尬。
...
就算不考虑上面的几条，那至少得遵守到第三范式吧？

数据库应用的时候还有很多要了解，比如一个表关联是什么意思，事务是什么东西，应用场景在哪里。

而我发现有些人都被ORM惯坏了，写SQL都生疏了。

好了，一个前端工程师做到这份上应该是差不多了吧。后面的交给DBA好了吧。

## Server

做一个独立前端自然少不了和server打交道。关于Nginx至少得会安装。然后会写简单的配置文件。这样，我们可以自己上一个网站了。用了Node还得学学反向代理的东西。

然而到这里哪里能满足爱折腾的前端？

HTTPS走起！从几家服务商中选择自己喜欢的，口碑好的，然后搞定证书。

跑了HTTPS依旧不满意，追新的我们怎么还用几十年前的HTTP/1.1？HTTP/2走起，自己下载编译安装Nginx，然后调参数，改配置。

这个阶段得学会Linux系编译，SSL/TLS的知识，加密算法，HTTP。。。
...
一万个人告诉我算法和数据结构才是计算机科学的核心，而算法和数据结构的核心是数学。
...
我在实习的第一个月在做module load的一些方案，因为对算法了解不深，写了很多操蛋的代码。好在后来各种重构没让我晚节不保。

前端多多少少会接触 canvas 和 WebGL 这些关于图形学的东西。可是如果线性代数学的不到家怎么可能写得好图形学的代码！

至于那些数据结构对于前端应用可能并不是很明显，然而一旦碰到将是致命的。

所以一个合格的前端算法还得过关。

## 工程化

工程化是我一直觉得前端的最最困难的地方。其严重性远超算法数据结构。
...
到现在，仍旧没有一个稳定的最好的解决方案。

这方面我了解还是不多，但是我认为如果有人能推出优秀的前端工程化方案，则必定是大师。
